...
def fix_sentencepiece_gguf(saved_location):
    ...
    logger.warning(
        f"Extending {saved_location}/tokenizer.model with added_tokens.json.\n"\
...
def _load_correct_tokenizer(
    ...
    try:
        ...
        pass
        # print(
        #     f"{tokenizer_name} has no tokenizer.model file.\n"\
    ...
    # Ignore Mistral ones - they're a bit weird to handle!
        ...
        if assert_same_tokenization(slow_tokenizer, fast_tokenizer):
            ...
            logger.warning(f"Will load {tokenizer_name} as a legacy tokenizer.")
...
def load_correct_tokenizer(
    ...
    # Also check Llama-2 old style models
        ...
        if old_chat_template is not None and chat_template is None:
            raise RuntimeError(
                "Fixing chat template failed - please file a report immediately!"
            )
        pass
    pass
    ...
    if no == yes:
        ...
        if "{% if add_generation_prompt %}" not in chat_template:
            ...
            if "{% if add_generation_prompt %}" not in new_chat_template:
                raise RuntimeError(
                    f"The tokenizer `{tokenizer.name_or_path}`\n"\
                    "does not have a {% if add_generation_prompt %} for generation purposes.\n"\
                    "Please file a bug report immediately - thanks!"
                )
            else:
                logger.warning_once(
                    "We successfully patched the tokenizer to add a {% if add_generation_prompt %} to the chat_template.\n"\
    ...
    for j, index in enumerate(added_tokens_fast.keys()):
        if index >= max_embedding_size:
            ...
            try:
                ...
                logger.warning_once(
                    "Tokenizer is most likely buggy, and Unsloth failed to repair it.\n"\
...
def fix_untrained_tokens(model, tokenizer, train_dataset, eps = 1e-16):
    ...
    if bad_not_trainable:
        raise ValueError(
            'Untrained tokens found, but embed_tokens & lm_head not trainable, causing NaNs. '\
    ...
    logger.warning(
        "Setting embed_tokens & lm_head untrained tokens to "\
...
def add_new_tokens(
    ...
    if method == "interpolation":
        print(
            "You are using interpolation to add new tokens.\n"\
