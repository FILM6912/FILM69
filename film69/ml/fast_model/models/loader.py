...
def __get_model_name(
    ...
    if not SUPPORTS_FOURBIT and model_name.lower() in INT_TO_FLOAT_MAPPER:
        ...
        logger.warning_once(
            f"Your transformers version of {transformers_version} does not support native "\
        ...
        # logger.warning_once(
        #     f": You passed in `{model_name}` which is a 4bit model, yet you set\n"\
        ...
        # logger.warning_once(
        #     f": You passed in `{model_name}` and `load_in_4bit = True`.\n"\
...
def _get_new_mapper():
    try:
        import requests
        new_mapper = "https://raw.githubusercontent.com/ai//main//models/mapper.py"
...
def get_model_name(model_name, load_in_4bit = True):
    ...
    if new_model_name is None and model_name.count("/") == 1 and model_name[0].isalnum():
        # Try checking if a new  version allows it!
        ...
        if upgraded_model_name is not None:
            raise NotImplementedError(
                f": {model_name} is not supported in your current  version! Please update  via:\n\n"\
            )
        pass
    pass
...
class FastLanguageModel(FastLlamaModel):
    ...
    def from_pretrained(
        ...
        *args, **kwargs,
        ...
        # In case the model supports tagging, add the  tag.
