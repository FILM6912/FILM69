    ...
    *args, **kwargs,
    ...
    if hasattr(self, "max_seq_length"):
        if seq_length > self.max_seq_length:
            logger.warning_once(
                f"Input IDs of length {seq_length} > the model's max sequence length of {self.max_seq_length}.\n"\
    ...
    for idx, decoder_layer in enumerate(self.layers):
        ...
        if offloaded_gradient_checkpointing:
            hidden_states = _Offloaded_Gradient_Checkpointer.apply(
                decoder_layer,
                hidden_states,
                mask,
                attention_mask,
        ...
        *args, **kwargs,
        ...
        if labels is not None:
            ...
            if not hasattr(self, "extra_ignored_labels"):
                # Fixes https://github.com/ai//issues/10
...
# Solves https://github.com/ai//issues/168
...
class FastLlamaModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if trust_remote_code:
            print(
                "WARNING `trust_remote_code` is True.\n"\
        ...
        statistics = \
           f"Fast {model_patcher.__name__[4:-5]} patching. Transformers = {transformers_version}.\n"\
           f"{gpu_stats.name}. Max memory: {max_memory} GB. Platform = {platform_system}.\n"\
        ...
        if os.environ.get("HF_HUB_ENABLE_HF_TRANSFER", "0") == "1":
            print("Fast downloading is enabled - ignore downloading bars which are red colored!")
        pass
        # Return old flag
        ...
        if (rope_scaling is None) and (max_seq_length > model_max_seq_length):
            ...
            logger.warning_once(
                f"{model_name} can only handle sequence lengths of at most "\
            ...
            if not has_rope_scaling:
                raise RuntimeError(
                    ...
                    "Please file a feature request at https://github.com/ai/."
                )
            pass
        ...
        try:
            ...
            raise RuntimeError('currently does not support multi GPU setups - but we are working on it!')
        pass
        ...
        try:
            ...
            if not torch.cuda.is_available():
                raise RuntimeError('We do not support AMD / Intel machines yet - it is a work in progress!')
        ...
        if n_total_devices > 1:
            logger.warning_once('currently does not support multi GPU setups - but we are working on it!')
        debug_info ="""
        debug_info = debug_info.split('\n')
        debug_info = "\n".join([debug_info[0]] + [spaces + x[8:] for x in debug_info[1:]])
        ...
        inner_training_loop = inner_training_loop.replace(
            ...
            "raise RuntimeError('TPUs are not yet supported!')"
        )
        inner_training_loop = inner_training_loop.replace(
            "self.accelerator.free_memory()",
            "self.accelerator.free_memory()\n" + \
            front_spaces + "if self.is_deepspeed_enabled:"\
            "raise RuntimeError('Deepspeed is not yet supported!')\n", 1,
        )
        ...
        if False:
            ...
            if name.startswith("/") and name.endswith("-bnb-4bit"):
                name = name[:len(name) - len("-bnb-4bit")]
                model.config.update({"_name_or_path" : name})
            pass
        pass

        # Log version for future fastpaths for inference
        model.config.update({"_version" : __version__})
    ...
    def get_peft_model(
        ...
        loftq_config        = {},
        temporary_location  = "__temporary_saved_buffers",
        **kwargs,
    ):
        ...
        if isinstance(model, PeftModelForCausalLM):
            ...
            if check_all:
                ...
                if "lm_head" in new_target_modules:
                    print("Casting lm_head to float32")
                ...
                raise TypeError(
                    "Your model already has LoRA adapters. Your new parameters are different."
                )
            pass
        pass
        ...
        if lora_dropout != 0:
            logger.warning_once(
                f"Dropout = 0 is supported for fast patching. You are using dropout = {lora_dropout}.\n"\
                f"will patch all other layers, except LoRA matrices, causing a performance hit."
            )
        pass

        if bias != "none":
            logger.warning_once(
                f"bias = `none` is supported for fast patching. You are using bias = {bias}.\n"\
                f"will patch all other layers, except LoRA matrices, causing a performance hit."
            )
        pass
        ...
        if not (type(init_lora_weights) is bool or \
            ...
            raise ValueError(
                '`init_lora_weights` must be either [True, False, "gaussian", "loftq"].'
            )
        pass
        ...
        if init_lora_weights == "loftq":
            ...
            if not SUPPORTS_LOFTQ:
                import peft
                raise RuntimeError(
                    f"Your PEFT version of {peft.__version__} does not support LoftQ init.\n"\
            ...
            if loftq_config == {}:
                ...
                logger.warning_once(
                    f"init_lora_weights = `loftq` is set, but `loftq_config` is None.\n"\
            ...
            if hasattr(model.config, "quantization_config"):
                raise ValueError(
                    "You are using `loftq` init, yet `load_in_4bit = True` was set.\n"\
        ...
        if use_rslora:
            if not SUPPORTS_RSLORA:
                ...
                import peft
                raise RuntimeError(
                    f"Your PEFT version of {peft.__version__} does not support `use_rslora`.\n"\
                    "Please install PEFT 0.7.2 or higher.\n"\
                    "You can also install from source: `pip install git+https://github.com/huggingface/peft.git"
                )
            pass
        ...
        for module in target_modules:
            if module == "lm_head":
                # logger.warning_once(
                #     "`lm_head` should be placed in `modules_to_save` and not `target_modules`. "\
                ...
                # logger.warning_once(
                #     "`embed_tokens` should be placed in `modules_to_save` and not `target_modules`. "\
                #     "Luckily, we shall do it for you!"
                # )
        ...
        if hasattr(model, "_need_to_train_embeddings"):
            if not train_lm_head or not train_embed_tokens:
                print(
                    "You added new tokens but did not specify if you wanted to "\
        ...
        if modules_to_save is not None:
            for module in modules_to_save:
                if module == "lm_head":
                    ...
                    raise TypeError(
                        f"Module = {module} is not allowed. Only 'lm_head' and 'embed_tokens' is allowed."
                    )
            pass
        pass
        ...
        if use_gradient_checkpointing == "":
            if train_embed_tokens:
                print("Offloading input_embeddings to disk to save VRAM")
            ...
            if train_lm_head:
                print("Offloading output_embeddings to disk to save VRAM")
    ...
    def patch_peft_model(
        ...
        if not isinstance(model, PeftModelForCausalLM):
            raise TypeError(
                "Your model needs to call `.get_peft_model` first!"
            )
        pass
        ...
        for active_adapter in model.peft_config.keys():
            ...
            # [TODO] Bugs out!see https://github.com/ai//issues/492
            # model.peft_config[active_adapter].revision = f""
        pass

        from transformers.trainer import Trainer 
        if Trainer._inner_training_loop.__name__ != "_fast_inner_training_loop":
            raise RuntimeError(
                'currently does not work on multi GPU setups - sadly we are a 2 brother team so '\
        ...
        if lora_dropout == 0 and bias == "none":
            for idx, layer in enumerate(model.model.model.layers):
                ...
                if  hasattr(q_proj, "lora_A") and \
                    ...
                    if model_type != "qwen2":
                        logger.warning_once(
                            "Not an error, but cannot patch Attention layers with our manual autograd engine since either LoRA adapters\n"\
                            "are not enabled or a bias term (like in Qwen) is used."
                        )
                    pass
                pass
        ...
        logger.warning_once(
            f"{__version__} patched {len(model.model.model.layers)} layers with "\
