...
def _free_cached_model(model):
    ...
    for cached_repo in cached_repos:
        if cached_repo.repo_id == model.config._name_or_path:
            ...
            logger.warning_once(
                "Will remove a cached repo with size " + \
    ...
    if isinstance(layer, (Bnb_Linear4bit, Peft_Linear4bit, Peft_Linear)):
        ...
        if A is not None:
            ...
            if not torch.isfinite(maximum_element).item():
                raise ValueError(f"Merge failed.\n{name} has some elements = infinity.")
        pass
        W = W.t().to(dtype)
    else:
        W = layer.weight
...
def unsloth_save_model(
    ...
    if save_method == "merged_4bit":
        raise RuntimeError(
            "Merging into 4bit will cause your model to lose accuracy if you plan\n"\
    ...
    if save_method != "lora" and save_method != "merged_16bit" and save_method != "merged_4bit":
        raise RuntimeError(
            "You must select one of 3 options when saving models:\n"\
    ...
    if save_method == "merged_4bit":
        ...
        print("Merging 4bit and LoRA weights to 4bit...")
    ...
    if ((save_method == "lora") or (save_method == "merged_4bit")) and push_to_hub:
        if token is None:
            raise RuntimeError(
                "Pushing to HF requires a token. Pass `token = 'hf_....'`\n"\
                "Go to https://huggingface.co/settings/tokens."
            )
        pass

        if save_method == "lora":
            print("Saving LoRA adapters. Please wait...")
        elif save_method == "merged_4bit":
            print("Saving 4bit Bitsandbytes model. Please wait...")
        pass
    ...
    if push_to_hub and "/" in save_directory:
        ...
        logger.warning_once(
            f"You are pushing to hub, but you passed your HF username = {username}.\n"\
    ...
    print("Merging 4bit and LoRA weights to 16bit...")
    ...
    if safe_serialization is None:
        ...
        logger.warning_once(
            f"You have {n_cpus} CPUs. Using `safe_serialization` is 10x slower.\n"\
    ...
    if IS_KAGGLE_ENVIRONMENT or IS_COLAB_ENVIRONMENT:
        ...
        logger.warning_once(
            "Kaggle/Colab has limited disk space. We need to delete the downloaded\n"\
    ...
    for key, value in state_dict.items():
        ...
        if type(value) is not torch.Tensor:
            logger.warning_once(f"{key} is not a Tensor but a {type(value)}.")
        pass
    pass
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        print(f"Saving to organization with address {new_save_directory}")
    ...
    print("Saving model... This might take 5 minutes for Llama-7b...")
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        ...
        print("Uploading all files... Please wait...")
...
def install_llama_cpp_old(version = -10):
    ...
    if not os.path.exists("llama.cpp/quantize") and not os.path.exists("llama.cpp/llama-quantize"):
        raise RuntimeError(
            "The file 'llama.cpp/llama-quantize' or `llama.cpp/quantize` does not exist.\n"\
...
def save_to_gguf(
    ...
    if   isinstance(quantization_method, list):  pass
        ...
        raise TypeError("quantization_method can only be a string or a list of strings")
    pass
    
    # Check if bfloat16 is supported
    if model_dtype == "bf16" and not torch.cuda.is_bf16_supported():
        logger.warning(
            "Cannot convert to bf16 GGUF since your computer doesn't support it.\n"\
    ...
    for quant_method in quantization_method: 
        if quant_method.startswith("iq2"):
            raise RuntimeError("Currently iq2 type quantizations aren't supported yet - sorry!")
    pass
    ...
    pass
    logger.warning_once(f"Converting {model_type} model. Can use fast conversion = {use_fast_convert}.")
    ...
    for quant_method in quantization_method:
        ...
        if quant_method not in ALLOWED_QUANTS.keys():
            error = f"Quant method = [{quant_method}] not supported. Choose from below:\n"
            for key, value in ALLOWED_QUANTS.items():
                error += f"[{key}] => {value}\n"
    ...
    if   first_conversion == "f16"  : pass
        ...
        raise RuntimeError(
            f"`first_conversion` can only be one of ['f16', 'bf16', 'f32', 'q8_0'] and not `{first_conversion}`."
        )
    pass
    ...
    if quantize_location is not None and convert_location is not None:
        print("llama.cpp found in the system. We shall skip installation.")
    else:
        print("[0] Installing llama.cpp. This will take 3 minutes...")
        ...
        if os.path.exists("llama.cpp/convert-hf-to-gguf.py"):
            ...
            raise RuntimeError(
                "The file 'llama.cpp/convert-hf-to-gguf.py' or 'llama.cpp/convert_hf_to_gguf.py' does not exist.\n"\
                "But we expect this file to exist! Maybe the llama.cpp developers changed the name?"
            )
        pass

        if error != 0 or quantize_location is None or convert_location is None:
            print(f"llama.cpp error code = {error}.")
    ...
    for quant_method in quantization_method:
        if   quant_method == "f32":  strength = max(strength, 3)
            ...
            if   first_conversion == "f32"  : pass
                ...
                logger.warning_once(
                    "Using q8_0 for the `first_conversion` will lose a bit of accuracy, "\
    ...
    if not use_fast_convert and \
        ...
        # logger.warning_once("We must use f16 for non Llama and Mistral models.")
        # first_conversion = "f16"
    pass
    ...
    print(f"[1] Converting model at {model_directory} into {first_conversion} GGUF format.\n"\
    ...
    if not os.path.isfile(final_location):
        if IS_KAGGLE_ENVIRONMENT:
            raise RuntimeError(
                f"Quantization failed for {final_location}\n"\
        ...
        pass
    pass
    print(f"Conversion completed! Output location: {final_location}")
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            print(f"[2] Converting GGUF 16bit into {quant_method}. This will take 20 minutes...")
            ...
            if not os.path.isfile(final_location):
                if IS_KAGGLE_ENVIRONMENT:
                    ...
                    raise RuntimeError(
                        "Quantization failed! You might have to compile llama.cpp yourself, then run this again.\n"\
                        "You do not need to close this Python program. Run the following commands in a new terminal:\n"\
                        "You must run this in the same folder as you're saving your model.\n"\
                        "git clone --recursive https://github.com/ggerganov/llama.cpp\n"\
                        "cd llama.cpp && make clean && make all -j\n"\
...
def unsloth_save_pretrained_merged(
    ...
    if tokenizer is None:
        logger.warning_once(
            "You're not saving a tokenizer as well?\n"\
...
def _determine_username(save_directory, old_username, token):
    ...
    if "/" not in save_directory:
        ...
        try: 
            ...
            raise RuntimeError(f"{save_directory} is not a Huggingface directory.")
    else:
...
def upload_to_huggingface(
    ...
    if file_location is not None:
        ...
        if len(ftevent_files) > 0:
            print("Uploading tensorboard files... Please wait...", file_location + "*out.tfevents*")
...
def fix_tokenizer_bos_token(tokenizer):
    ...
    if (tokenizer("A").input_ids[0] == getattr(tokenizer, "bos_token_id", None)):
        if chat_template is not None and \
            ...
            fix_bos_token = True
            logger.warning(
                f"##### The current model auto adds a BOS token.\n"\
                "##### Your chat template has a BOS token. We shall remove it temporarily."
            )
...
def unsloth_save_pretrained_gguf(
    ...
    """
    if tokenizer is None:
        raise ValueError("Saving to GGUF must have a tokenizer.")
    ...
    if type(model_dtype) is str:
        ...
        raise TypeError("Model dtype can only be float16 or bfloat16")
    pass
    ...
    if modelfile is not None:
        ...
        pass
        print(f"Saved Ollama Modelfile to {modelfile_location}")
    pass

    if fix_bos_token:
        logger.warning(
            f"##### The current model auto adds a BOS token.\n"\
            "##### We removed it in GGUF's chat template for you."
        )
    pass

    if push_to_hub:
        print("Uploading GGUF to Huggingface Hub...")
...
def unsloth_convert_lora_to_ggml_and_push_to_hub(
    ...
    print(f"Converting auto-saved LoRA adapters at {lora_directory_push} to GGML format.")
    ...
    print("Uploading GGML file to Hugging Face Hub...")
    username = upload_to_huggingface(
        self, repo_id, token,
...
def unsloth_convert_lora_to_ggml_and_save_locally(
    ...
    print(f"Converting auto-saved LoRA adapters at {save_directory} to GGML format.")
    print(f"The output file will be {output_file}")
